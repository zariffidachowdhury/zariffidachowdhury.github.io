...
This is the source of https://zariffidachowdhury.github.io/ (GitHub Pages, user site, `main` branch, root). Hand-written HTML and CSS, no framework, no build step, no external requests.
...
- The page pins a SHA-256 for every CSS, font, image, JS and résumé file (see the colophon). After changing any file in `css/`, `fonts/`, `img/`, `js/` or either résumé file, run `python3 tools/integrity.py` from this folder before committing. It rewrites the manifest and the ledger rows inside `index.html`. Editing only the text of `index.html` does not need a re-pin.
...
- One accent color only (`--ox` in `css/site.css`). Keep the light paper theme, radius 0, no shadows, no gradients.
