...
The page also checks itself: `tools/integrity.py` pins a SHA-256 hash of every file the page loads into `index.html` and stamps the stylesheet and script tags with Subresource Integrity attributes, so the browser refuses a file that does not match. `js/audit.js` then re-hashes every file in the visitor's browser and reports the result in the colophon.
...
```bash
python3 tools/integrity.py   # re-pin the hashes and SRI attributes
python3 tools/check.py       # one command that says whether it is fit to commit
```
